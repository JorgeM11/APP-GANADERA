...
export default function InventarioPage() {
  ...
  // COMPONENTE BÚSQUEDA MÓVIL (Visible solo en teléfonos)
  ...
  // COMPONENTE BÚSQUEDA DESKTOP (Visible solo en PC/Tablet, con diseño anterior)
  ...
  return (
    <main className="min-h-screen bg-[#F7F7F2] font-sans pb-28 relative">
      ...
      {/* OVERLAY DE FONDO OSCURO (Solo visible cuando el botón está abierto) */}
      {/* z-20 para tapar el header, cursor-pointer para invitar a hacer clic y cerrar */}
      {isFabOpen && (
        <div 
      ...
      {/* HEADER HÍBRIDO */}
      {/* Ajustado a z-10 para que el overlay oscuro quede por encima al abrir el menú */}
      <header className="bg-white md:bg-[#1B4820] px-4 pt-6 pb-4 md:py-4 md:px-8 sticky top-0 z-10 shadow-sm md:shadow-md transition-colors duration-300">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#1B4820] md:text-white tracking-tight">
            Inventario Bovino
          </h1>
      ...
      {/* CONTENIDO PRINCIPAL: Grid de Animales */}
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-5 md:mt-8 relative z-0">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5 md:gap-6">
          {mockAnimals.map((animal) => (
            <article 
              key={animal.id} 
              className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-200 flex flex-col"
            >
      ...
      {/* BOTÓN DE ACCIONES FLOTANTE (FAB) */}
      {/* z-30 para asegurar que esté por encima del overlay oscuro (z-20) */}
